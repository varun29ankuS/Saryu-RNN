...
at context 128 and evaluated up to context 8192). It reports only Saryu's own measurements;
...
\paragraph{Evaluation.} To see how the model uses context longer than it was trained on, each number
below is measured with the model reading a window of $L\in\{128,512,2048,8192\}$ characters and scoring
only the \emph{last 128} characters of the window, over 64 windows drawn at random from the evaluation
text (8,192 scored characters per number). The windows are drawn afresh for each $L$, so neighbouring
columns score different characters. The unit is bits per character (bpc); lower is better.
...
\midrule
Saryu 5M (checkpoint shipped with the code) & 1.782 & 1.671 & 1.706 & 1.606 \\
Saryu 5M, base recipe, three seeds & 1.782--1.793 & 1.671--1.681 & 1.706--1.730 & 1.604--1.618 \\
Saryu 25M & \textbf{1.643} & \textbf{1.516} & \textbf{1.562} & \textbf{1.467} \\
\bottomrule
\end{tabular}
\caption{Character-level results on our enwik8 split. Every model was trained at context 128. The
three-seed row (base 5M recipe without the value embedding and learning-rate multipliers) shows the
seed-to-seed spread.}
\label{tab:bpc}
\end{table}
...
\begin{semilogxaxis}[width=7.4cm, height=5.0cm, log basis x=2,
  ...
  ymin=1.42, ymax=1.92, legend style={font=\scriptsize, draw=none, fill=none}, legend pos=north east,
  tick label style={font=\scriptsize}, label style={font=\small}]
  \addplot[thick, mark=square*, accB] coordinates {(128,1.643) (512,1.516) (2048,1.562) (8192,1.467)};
  \addlegendentry{Saryu 25M}
  \addplot[thick, mark=*, accA] coordinates {(128,1.782) (512,1.671) (2048,1.706) (8192,1.606)};
  \addlegendentry{Saryu 5M}
  \addplot[name path=hi, draw=none, forget plot] coordinates {(128,1.793) (512,1.681) (2048,1.730) (8192,1.618)};
  \addplot[name path=lo, draw=none, forget plot] coordinates {(128,1.782) (512,1.671) (2048,1.706) (8192,1.604)};
  \addplot[accA!25, forget plot] fill between[of=hi and lo];
\end{semilogxaxis}
...
\end{axis}
\end{tikzpicture}
\caption{\textbf{Left:} bits per character as the model reads longer context than it was trained on;
the shaded band is the three-seed spread of the 5M base recipe. \textbf{Right:} what the 25M model's
...
\begin{itemize}[leftmargin=*,itemsep=2pt]
  \item \textbf{Longer context helps although it was never trained on.} Both models score the last 128
        characters of a window better when the window is 8,192 characters long than when it is 128
        long ($1.643\to1.467$ bpc at 25M). A recurrent state has no position embedding to fall off the end of,
        and the bound of Proposition~\ref{prop:bounded} keeps the state in range at any length.
  \item \textbf{The curve is not monotone.} Context 2048 scores worse than 512 for both models and for
        every seed. Because the windows differ between columns, part of this can be sampling (8,192 scored
        characters per number); whether some of it is real is open and is on the list for the next
        version, which will score identical characters across lengths.
  \item \textbf{Scale helps at every length.} Five times the parameters and twice the training
        characters lower bpc by $0.14$--$0.16$ at every context length; the seed spread at 5M is about
        $0.01$--$0.02$.
\end{itemize}
...
and a transformer --- with several seeds, a second corpus, identical scored characters across context
lengths, state-tracking tasks, and throughput. The design questions above (Section~\ref{sec:why}) are
