...
operator, because a single reflection hides almost nothing. Raising $n_h$ for a subset of heads fixes
it with the operator already in the architecture, no second state and no second kernel
(Figure~\ref{fig:memory}). The design that follows is heterogeneous: tracking heads at $n_h=2$, where
near-identity transports compose across tokens, and storage heads at $n_h\approx d/4$ to $d/2$, where
the transport is close to Haar-random and conceals a value until its key returns.
