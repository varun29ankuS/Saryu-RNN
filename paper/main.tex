...
at context 128, and measured to use about 512 characters of context and no more). It also identifies what this transport cannot do and why: the expected overlap of a state with its
image under an orthogonal map is the normalised trace, a single Householder has trace $d-2$ --- the
least-hiding non-trivial element of the orthogonal group --- and $k$ reflections give overlap
$e^{-2k/d}$, so the same operation that tracks at $k=2$ stores associations only near $k\approx d/2$.
It reports only Saryu's own measurements;
...
$0.779\pm0.380$ on $S_3$ over three seeds, above the bound that no commuting transport can cross. A
\emph{reflection} transport --- the operation this paper is about, with $n_h=4$ --- reaches
$0.780\pm0.382$ on the same task, so the gap is not an artefact of the butterfly
parameterisation.\footnote{Repository \texttt{evidence/ceiling.py} and
\texttt{evidence/ceiling\_reflection.py}, logs \texttt{evidence/results/v2.txt} and
\texttt{evidence/results/ceiling\_reflection.txt}; the bounds are recomputed by exact enumeration in
\texttt{evidence/verify\_paperB.py}. The spread is not noise: the three reflection seeds return
$1.000$, $0.339$, $1.000$, and $1/3$ is a rung of $S_3$ --- the seed that misses the ceiling has
learned a quotient rather than failed randomly. Being non-commuting removes a ceiling; it does not
supply an optimiser that reaches above it. Paper B treats where such runs land.} Two reflections about different mirrors already fail to commute
...
\subsection{What the transport cannot do: binding is a character}
\label{sec:binding}

The same model that composes an ordered 48-step chain perfectly at every length (1.000, flat to a
distance of 144 tokens) cannot retrieve one key--value pair out of four at any gap, at any learning
rate: the loss sits at $\ln 64$ for 2000 steps. The reason is a trace identity, and it also explains
why the failure gets \emph{worse} as the model is made wider.

For an orthogonal map $g$ acting on the sphere, the expected overlap of a random unit vector with its
image is the normalised trace,
\begin{equation}
  \mathbb{E}\,[\,\mathbf{v}^\top g\,\mathbf{v}\,] = \operatorname{tr}(g)/d ,
  \label{eq:trace}
\end{equation}
so \emph{how much a binding operation hides a value is a character}. Writing $g\in O(d)$ by its
rotation angles, $\operatorname{tr}(g)=\sum_i 2\cos\theta_i$: a single Householder has one angle at
$\pi$ and the rest at zero, giving $\operatorname{tr}=d-2$ and overlap $1-2/d$ ($0.978$ at $d=93$),
while a phase rotation or a permutation has $\operatorname{tr}\approx0$ and overlap $\approx 0$. A
reflection is thus the least-hiding non-trivial element of the whole group. A product of $k$
reflections multiplies the expected trace by $(1-2/d)$ each time, so
\begin{equation}
  \mathbb{E}\,[\,\mathbf{v}^\top g_k\,\mathbf{v}\,] \;\approx\; e^{-2k/d} .
  \label{eq:decay}
\end{equation}
...
\toprule
$k$ & measured overlap & $e^{-2k/d}$ & 4-pair recall & 8-pair recall \\
\midrule
1  & 0.968 & 0.969 & 0.373 & 0.187 \\
2  & 0.945 & 0.939 & 0.453 & 0.200 \\
4  & 0.872 & 0.882 & 0.573 & 0.233 \\
8  & 0.777 & 0.779 & 0.807 & 0.480 \\
16 & 0.602 & 0.607 & 0.953 & 0.673 \\
32 & 0.358 & 0.368 & \textbf{1.000} & 0.853 \\
48 & 0.218 & 0.223 & 0.993 & \textbf{0.933} \\
\midrule
\multicolumn{3}{r}{chance} & 0.250 & 0.125 \\
\bottomrule
\end{tabular}
\caption{Binding by $k$ reflections at head dimension $64$, weights set by hand and no training.
Overlap follows Eq.~\ref{eq:decay} to three decimals, and capacity follows the overlap: the same
mechanism that stores nothing at $k=2$ stores four associations perfectly at $k=32$. Verified
independently at $d=32$ and $d=128$ across three seeds, and against exact traces of the assembled
product.}
\label{tab:trace}
\end{table}

Table~\ref{tab:trace} measures both sides. Capacity tracks the trace, so \emph{transport and binding
are one operation at two ends of one knob}: small $k$ gives near-identity nudges that compose across
tokens, and $k\approx d/2$ gives a near-Haar isometry that conceals a value until the matching key
returns. Saryu runs $n_h=2$ at head dimension 40 and 93, i.e.\ $2k/d\approx0.04$ --- the extreme
tracking end --- which is why it composes perfectly and stores nothing. DeltaProduct introduced $n_h$
as a state-tracking expressivity knob~\cite{siems2025}; Eq.~\ref{eq:decay} says it is simultaneously
the binding-capacity knob, and that the two regimes are different jobs for the same parameter.

Two costs are real. Storage heads are $O(kd)$ per token, so $k\approx d/2$ costs $O(d^2)$ --- the
compute of a matrix-state memory, though the state itself stays $O(d)$ per head. And the kernel's
per-chunk system is $(Cn_h)\times(Cn_h)$, so $n_h=32$ at chunk 8 solves $256\times256$ rather than
$16\times16$; the kernel remains exact at those sizes (measured $7\times10^{-6}$ against the
step-by-step recurrence), so no new kernel is required.
...
\paragraph{The chunk size, then perhaps a fused kernel.} The kernel is exact for \emph{any} chunk
size, so the chunk is a free performance knob, and on a T4 it is worth a great deal: one 25M layer's
forward and backward at context 512 falls from 131\,ms at chunk 8 to 21\,ms at chunk 64, taking a
whole training step from 695\,ms to 300\,ms, with differences at floating-point noise. That also
weakens the case for hand-writing a fused kernel: at the shipped chunk the recurrence was 35--73\% of
a step, and at the best chunk it is 17--53\%.

\paragraph{Storage: more reflections, not a second memory.} An earlier version of this draft proposed
adding a matrix memory beside the tracker, on the assumption that a vector state cannot hold many
independent facts~\cite{arora2023zoology}. Section~\ref{sec:binding} shows that assumption was wrong
in an instructive way. The state stores associations perfectly well; what fails is the binding
operator, because a single reflection hides almost nothing. That suggested a heterogeneous layer ---
tracking heads at $n_h=2$, where near-identity transports compose across tokens, and storage heads at
$n_h\approx d/4$ to $d/2$, where the transport is close to Haar-random and conceals a value until its
key returns (Figure~\ref{fig:memory}).

\paragraph{We tested that, and it does not work.} Sweeping $n_h$ in a trained model, with everything
else held fixed, the trace follows the law closely ($0.818$, $0.611$, $0.364$, $0.122$ at
$n_h=2,4,8,16$, against a predicted $0.766$, $0.586$, $0.344$, $0.118$) --- and recall falls
monotonically, $0.113\to0.047\to0.020\to0.004$, ending below chance. More reflections made the memory
worse, not better, and the registered prediction of a $+0.20$ gain came out at $-0.109$.

The mismatch is structural rather than a matter of tuning. A bind/unbind store recovers a value by
applying its key's reflections a second time, an involution undoing the first; and it assumes the
bound pair is untouched between write and read. This recurrence does neither --- it never applies a
query's inverse, and every later token transports the \emph{whole} state, so a stored pair is
scrambled by everything that follows. At $n_h=2$ the trained model backs off to $\beta=1.56$, away
from the pure reflection, to keep the state intact; at $n_h=16$ no $\beta$ rescues it. The trace law
bounds what a product of reflections could store, and we no longer claim that raising $n_h$ turns
this architecture into a memory. A read path that applies the query's inverse, and some protection
for a stored pair from intervening tokens, would be a different architecture --- which is the honest
open problem, and the matrix memory this paragraph set out to avoid may yet be the simpler answer.

Two things that this does \emph{not} fix are worth stating. Recall still decays with distance
(0.234 at gap 0 against 0.062 at gap 64 under a curriculum), because the transport rotates the stored
superposition between write and read --- a frame drift that more reflections do not address. And at
$\beta=2$ the recurrence is a measure-preserving map of a compact sphere, so by Poincar\'e recurrence
it must return arbitrarily close to earlier states: more reflections buy separation, not escape.
Escaping needs dissipation ($\beta<1$), a non-compact state space, or a dimension high enough that the
recurrence time $\sim\epsilon^{-d}$ is irrelevant.

\begin{figure}[t]
\centering
\begin{tikzpicture}[node distance=6mm]
  \node[font=\small] (x) {token features $\bz_t$};
  \node[blk, fill=mixc, above left=9mm and 6mm of x, minimum width=42mm] (trk)
    {tracking heads, $n_h=2$\\{\scriptsize overlap $e^{-2k/d}\approx0.96$: near-identity nudges}\\
     {\scriptsize composes across tokens --- this paper}};
  \node[blk, fill=memc, above right=9mm and 6mm of x, minimum width=42mm] (mem)
    {storage heads, $n_h\approx d/2$\\{\scriptsize overlap $\approx0.37$: near-Haar isometry}\\
     {\scriptsize conceals a value until its key returns}};
  \coordinate (mid) at ($(trk.north)!0.5!(mem.north)$);
  \node[blk, fill=statec, above=10mm of mid, minimum width=52mm] (st)
    {one vector state per head, one kernel\\{\scriptsize same operator, same recurrence, $O(d)$ state}};
  \node[font=\small, above=7mm of st] (out) {to the residual stream};
  \draw[arr] (x) -| (trk); \draw[arr] (x) -| (mem);
  \draw[arr] (trk) |- (st); \draw[arr] (mem) |- (st); \draw[arr] (st) -- (out);
  \node[font=\scriptsize, gray, right=3mm of x] {$n_h$ sets the trace, the trace sets the job};
\end{tikzpicture}
\caption{What the trace law implies for the design. Rather than bolting a matrix memory beside the
tracker, the same reflection transport does both jobs at two settings of $n_h$: tracking where the
transport is nearly the identity, storage where it is nearly Haar-random. One operator, one exact
kernel, and the state stays a vector. An earlier version of this draft proposed the matrix memory
instead; Section~\ref{sec:binding} is why it is not needed.}
\label{fig:memory}
\end{figure}
