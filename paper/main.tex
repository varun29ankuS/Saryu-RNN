...
reflection transport of the kind used here, with $n_h=4$, reaches $0.778$ on the same task, so the
gap is not specific to the butterfly.\footnote{Repository \texttt{evidence/ceiling.py}, logs
\texttt{evidence/results/v2.txt} and \texttt{archive/results/abelian\_ladder.txt}; the bounds
themselves are recomputed by enumeration in \texttt{evidence/verify\_paperB.py}. Two caveats travel
with this: the $\pm0.380$ spread means some seeds solve the task and others fall below the ceiling,
and on larger groups ($Q_8$, $A_4$, $S_4$) the reflection transport lands \emph{below} the abelian
ceiling. Being non-commuting removes a ceiling; it does not supply an optimiser that reaches what lies
above it. Paper B treats where those runs land instead.} Two reflections about different mirrors already fail to commute
