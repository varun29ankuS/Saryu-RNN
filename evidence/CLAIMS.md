...
| 38 | The delta rule's 0.38 residual against an attention layer is intrinsic to it | **wrong, and the sweep that suggested it was measuring the wrong axis.** Every sizing arm -- 2x and 4x state, 2 and 3 writes per token, both together -- landed inside the seed spread, and 3.9x the parameters bought nothing. But an output gate plus an A_log decay cut the residual from 0.382 to 0.228, 4.5x the spread. It was missing CONTROLS, not capacity. Neither component does it alone: the decay spread is worthless without a gate to suppress irrelevant heads, which is also why the three hand-installed timescale ladders in gate_timescales.txt collapsed -- all three were the decay half with no gate | reading Qwen and Kimi's source after an independent review refuted three of my four claims about them |
| 39 | Operation order inside the gated norm is a detail | **no.** Gate-then-norm (the Mamba-2 order, written from memory) scored 1.035 -- worse than a fitted linear map, i.e. worse than not having the layer. Norm-then-gate, which is what Qwen3NextRMSNormGated does under a source comment reading "Norm before gate", scores 0.334. Identical parameters and component. Had the first version finished, the recorded conclusion would have been that the output gate is harmful, with a number to support it | asking for the sources to be read directly rather than through a summariser |
| 36 | Saryu solves state tracking where other architectures cannot | **half retracted, and it was our strongest capability claim.** A GRU with pre-norm and residuals scores 1.000/1.000 on the same S_3 word problem Saryu scores 1.000/1.000 on; the transformer gets 0.727/0.703. What survives is "a recurrence beats attention at state tracking", which the complexity theory predicts -- attention is in TC^0, the word problem is NC^1-complete, regular languages are not all in TC^0. What does not survive is any implication that THIS recurrence is distinguished by it: a GRU is a nonlinear recurrence, was never in TC^0, and matches us. The control was never run until 2026-09-22 | running the missing baseline instead of the flattering one |
| 37 | The chunk-parallel kernel is a real advantage over a sequential RNN | **holds, and it is now the whole distinct claim.** Step time against sequence length at constant tokens per step: the GRU/Saryu ratio goes 1.36x, 1.80x, 2.28x, 2.95x, 3.70x at ctx 128 to 2048 -- monotone, and 3.7x at 2048. Registered as a live falsifier beforehand because nn.GRU is a fused C++ kernel while ours is a Python loop over small ops, and the same measurement against a transformer had gone the other way (our disadvantage widened, 3.9x to 5.8x) | `evidence/parallel_advantage.py` |
