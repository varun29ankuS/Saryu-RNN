...
class SaryuV3Block(nn.Module):
    ...
    @staticmethod
    def _recall_chunk(q, k, v, beta, betaw, alpha, chunk):
        """Chunk-parallel delta rule. EXACT -- the same recurrence, not an approximation.

        A staticmethod because it touches no instance state -- verified, not assumed. That makes
        it reusable outside a block, which is what an attention-layer surrogate needs: distilling
        a softmax transformer into this recurrence means running the delta rule on projections
        that belong to somebody else's model. `self._recall_chunk(...)` still resolves, so every
